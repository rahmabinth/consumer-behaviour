...
## Methodology
The language and environment used for this analysis is R [@citeR], alongside the tidyverse (@tidyverse), rstanarm (@rstanarm), cansim [@cansim], ggplot2 [@ggplot2], scales [@scales], modelsummary [@modelsummary], and marginaleffects [@marginaleffects] packages. 
...
The goal of our model is to forecast the total expenditure in a household in a year, based solely on the Consumer Price Index of all-items, and the number of people in Ontario earning an income. We run the model in R [@citeR] using the `rstanarm` package of @rstanarm. We use the default priors from `rstanarm`. Background details and diagnostics are included in [Appendix -@sec-model-details].
...
* $y_i$ is the continuous outcome variable, representing the total expenditure in Canadian dollars in year $i$. 
* $cpi_i$ is the predictor variable, representing the annual Consumer Price Index (CPI) for year $i$
* $income_i$ is the number of people earning an income in year $i$.
...
Our model results are summarized in @tbl-modelresults. The table compares our two models, each predicting the outcome variable (total expenditure), based on either CPI only, and CPI and the number of income earners. 
